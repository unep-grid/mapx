...
function connectRedis(session) {
  ...
  class RedisStore extends Store {
    ...
    set(sid, sess, cb = noop) {
      ...
      if (ttl > 0) {
        console.log("Session saved. key", key, "value", value);
        clientRedis.set(key, value, opt, cb);
      } else {
    ...
    touch(sid, sess, cb = noop) {
      ...
      let key = this.prefix + sid;
      clientRedis.expire(key, this._getTTL(sess), (err, ret) => {
        if (err) return cb(err);
    ...
    destroy(sid, cb = noop) {
      let key = this.prefix + sid;
      clientRedis.del(key, cb);
    }

    clear(cb = noop) {
      this._getAllKeys((err, keys) => {
        if (err) return cb(err);
        clientRedis.del(keys, cb);
      });
    }
    ...
    all(cb = noop) {
      ...
      this._getAllKeys((err, keys) => {
        ...
        clientRedis.mget(keys, (err, sessions) => {
          if (err) return cb(err);
    ...
    _scanKeys(keys = {}, cursor, pattern, count, cb = noop) {
      ...
      clientRedis.scan(args, (err, data) => {
        if (err) return cb(err);
