          ...
          for (const tblTemp of tblsTemp) {
            for (const fileRegex of tblTemp.filesRegex) {
              ...
              if (toImport) {
                ...
                await mxCopyStreamToTable(readStream, tblTemp.table.name);
              }
            }
          }
        }
...
async function dropTemp() {
  ...
  for (const tblTemp of tblsTemp) {
    const qDropTable = tblTemp.sqlTbl.drop().ifExists().toQuery();
...
async function createFinal() {
  ...
  const qDrop = tFinal.drop().ifExists().toQuery();
