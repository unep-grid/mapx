...
function validateAbstract(meta,n) {
  ...
  if(!hasAbstract) reasons.push("validate_meta_invalid_abstract");
...
function validateTitle(meta,n) {
  ...
  if( !hasTitle ) reasons.push("validate_meta_invalid_title");
...
function validateKeywords(meta,n) {
  ...
  if( !hasKeywords ) reasons.push("validate_meta_no_keyword");
  if( !hasValidKeywords ) reasons.push("validate_meta_invalid_keyword");
...
function validateContact(meta) {
  ...
  if( !hasContact ) reasons.push("validate_meta_no_contact");
  if( !hasValidContact ) reasons.push("validate_meta_invalid_contact_email");
...
function validateIssuance(meta) {
  ...
  var hasPeriodicity = h.isStringRange(h.path(meta, "temporal.issuance.periodicity", ""),3);
  var hasReleasedAt = h.isDateString(h.path(meta, "temporal.issuance.released_at", ""));
  var hasModifiedAt = h.isDateString(h.path(meta, "temporal.issuance.modified_at", ""));

  if( !hasPeriodicity ) reasons.push("validate_meta_invalid_issuance_periodicity");
  if( !hasReleasedAt ) reasons.push("validate_meta_invalid_issuance_released_at");
  if( !hasModifiedAt ) reasons.push("validate_meta_invalid_issuance_modified_at");
...
function validateLicense(meta,n) {
  ...
  var hasValidLicenses = h.all(licenses.map( l => { 
    return h.isStringRange(l.text,n) && 
      h.isStringRange(l.name,n);
  }));

  if( ! hasLicense ) reasons.push("validate_meta_no_license");
  if( ! hasValidLicenses ) reasons.push("validate_meta_invalid_license");
...
function validateSource(meta) {
  ...
  if( ! hasSources ) reasons.push("validate_meta_no_source");
  if( ! hasValidSources ) reasons.push("validate_meta_invalid_source");
