...
export function RadialProgress(selector, c) {
  ...
  prog.init = function() {
    ...
    c.strokeColor =
      c.strokeColor ||
      mx.settings.colors.mx_ui_text_faded ||
      'rgba(53,53,53,0.5)';
    c.svg = `
      <svg style="transform: rotate(270deg)" height="${c.height}" width="${
      c.height
    }">
      <circle r="${c.radius}" cx="${c.center}" cy="${
      c.center
    }" fill="transparent" stroke-dashoffset="0px" stroke-dasharray="${
      c.circumference
    }" style="stroke-width:${
      c.stroke
    };fill:transparent;transform:rotate(0.1deg);stroke: ${
      c.strokeColor
    };"></circle>
    </svg>`;
  ...
  prog.isInit = function() {
    return prog._is_init === true;
  };
