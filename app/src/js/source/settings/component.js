...
export class MxSourceSettingsElement extends HTMLElement {
  ...
  async load() {
    ...
    if (!response?.ok) throw new Error(response?.error || this.labels.source_settings_load_error);
  ...
  renderError(error) {
    ...
    this.setActionsDisabled(true);
    const { el } = this.elements;
    const message = error?.message || this.labels?.source_settings_load_error || "Unable to load source settings";
    const retry = el(
      "button",
      {
        type: "button",
  ...
  async render() {
    ...
    const fields = el(
      "section",
      { class: "mx-source-settings__fields" },
      [
        title.wrapper,
        id.wrapper,
        editor.wrapper,
  ...
  ensureActions() {
    if (this.actionElements?.save && this.actionElements?.remove) return;
    const { el } = this.elements;
    this.actionElements = {
      save: el("button", {
        type: "button",
  ...
  makeGlobalField() {
    ...
    input.checked = this.overview.constraints.forceGlobal || this.overview.source.global;
    input.disabled =
      !this.overview.permissions.canSetGlobal || this.overview.constraints.forceGlobal;
  ...
  makeUsageDetails(category) {
    ...
    details.addEventListener("toggle", () => {
      if (details.open && !this.usage.has(category)) this.loadUsage(category, content);
    });
    if (count === 0) details.hidden = true;
    return details;
  }

  async initializeSelects(refs) {
    const TomSelect = await moduleLoad("tom-select");
    if (!this.isConnected || this.refs !== refs) return;
  ...
  destroySelects() {
    for (const select of this.selects || []) select.destroy?.();
    this.selects = [];
  }
  ...
  renderConstraints() {
    ...
    if (constraints.forceGlobal) messages.push(this.labels.source_settings_force_global);
    if (constraints.protectPublisherReaders) messages.push(this.labels.source_settings_publishers_required);
    if (constraints.blockDelete) messages.push(this.labels.source_settings_delete_blocked);
    const { el } = this.elements;
    this.refs.warnings.replaceChildren(
      el("i", { class: ["fa", "fa-exclamation-triangle"], "aria-hidden": "true" }),
      el("ul", messages.map((message) => el("li", message))),
    );
  ...
  async loadUsage(category, content, offset = 0) {
    ...
    try {
      const response = await ws.emitAsync(
        ...
        { idSource: this.idSource, category, language: this.language, limit: PAGE_SIZE, offset },
        30 * 1000,
      );
      if (!response?.ok) throw new Error(response?.error || this.labels.source_settings_load_error);
  ...
  renderUsage(category, content, state) {
    ...
    if (state.error) {
      content.replaceChildren(el("p", { class: "alert alert-danger" }, state.error));
      return;
    }
    const table = el("table", { class: ["table", "table-condensed", "table-striped"] });
    table.append(
      el("thead", el("tr", [
      ...
      el("tbody", state.rows.map((row) => el("tr", [
    ...
    if (state.rows.length < (state.total || 0)) {
      nodes.push(el("button", {
        ...
        on: { click: () => this.loadUsage(category, content, state.rows.length) },
  ...
  async save() {
    ...
    if (this.overview.permissions.canSetGlobal) changes.global = this.refs.global.checked;
    try {
      const response = await ws.emitAsync(
      ...
      if (!response?.ok) {
        ...
        throw new Error(response?.error || this.labels.source_settings_load_error);
      }
      triggerUpdateSourcesList();
      triggerSourceSettingsChanged();
      await this.load();
      ...
      this.setFeedback(error?.message || this.labels.source_settings_load_error, true);
    } finally {
      this.setBusy(false);
    }
  }
  ...
  async remove() {
    const manager = getMapxWindowManager(this.applicationRoot || this.ownerDocument.body);
  ...
  setBusy(busy) {
    ...
    this.refs.global.disabled = busy || !this.overview.permissions.canSetGlobal || this.overview.constraints.forceGlobal;
    for (const select of this.selects) select[busy ? "disable" : "enable"]?.();
  }
