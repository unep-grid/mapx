...
*/
mgl.helper.saveViewsList = function(o){

  var m = mgl.maps[o.id];
  var view, views, sourceId, sourceExists, sourceStore;

  if(m){
    if( o.add && m.views && ! (o.viewsList instanceof Array) ){
      m.views.push( o.viewList ) ;
      views = [o.viewList];
    }else{
       m.views = o.viewList;
      views = m.views;
    ...
    for( var i = 0 ; i < views.length ; i++ ){
      ...
      if(view.data.source){
        ...
        sourceId = view.id + "-SRC";
        sourceExists = !!m.map.getSource(sourceId);

        if( sourceExists ) {
          m.map.removeSource( sourceId ) ;
        }

        //console.log("Add source id : " + sourceId );
        m.map.addSource(
          sourceId,
          view.data.source
        );
...
mgl.helper.updateViewOrder = function(o){
  ...
  displayed.sort(
    function(a,b){
      posA = order.indexOf(a.split("__")[0]);
      posB = order.indexOf(b.split("__")[0]);
      return posA-posB;
    });
    ...
    t = [
      ...
      },
      {
        ...
        action : function(){
          ...
          mx.util.classAction({
            selector : "#" + el.dataset.view_action_target,
            action : "toggle"
          });
        }
      },
...
mgl.helper.renderViewsList = function(o){
  ...
  if( ! (o.views instanceof Array )) o.views = [o.views];
  ...
  if( !o.views || o.views.constructor !== Array ||  o.views.length < 1 || !mx.templates.viewList ){
    ...
    if( ! m.tools.viewListJs ){
      m.tools.viewsListJs = new List( elViewsContainer, {
        valueNames: mgl.helper.objectToArray(viewClasses),
 ...
 *
 */
mgl.helper.getLayerById = function(mapId, id) {
...
mgl.helper.filterViewValues = function(o){
  ...
  var view, views, idView, filterAll, varView, search, map, filt, valueMatch, valueStore = [], layers={}, idViewAll=[];
  var l, lay, origFilter, newFilter, filter;
  var operators = ["==",">=","<=",">","<"];
  var groupAnd = ["all"];
  var gOr, gAnd, op, rul, ops;

  /**
   * id can be a sub view : eg. MX-YN0ZZ-T2YUQ-64LLV__pattern_0 
   * Extracting base layer : MX-YN0ZZ-T2YUQ-64LLV
  */ 
  idView = o.idView.split("__")[0];
  views = mgl.helper.getKeyedViews(o);
...
mgl.helper.zoomToViewId = function(o){
  ...
  if ( hasMap ) {
    ...
    var sw = new mapboxgl.LngLat(extent.lng1, extent.lat1);
    var ne = new mapboxgl.LngLat(extent.lng2, extent.lat2);
 ...
 * @param {string} o.idView view list id
...
* @return {array}
*/
mgl.helper.objectToArray = function( obj ){
...
*     })
*/
mgl.helper.checkLanguage = function(o){
  
  var langs = o.languages? o.languages: mx.languages? mx.languages: ["en","fr","sp"];
...
$('document').ready(function() {
  ...
  Shiny.addCustomMessageHandler( 'mglSaveViewsList',mgl.helper.saveViewsList );
